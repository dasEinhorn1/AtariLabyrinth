  ...
  ;
  dim _BitOp_P1_P0_Dir = f
  dim _Bit0_P1_Dir_Up = f
  dim _Bit1_P1_Dir_Down = f
  dim _Bit2_P1_Dir_Left = f
  dim _Bit3_P1_Dir_Right = f


  ;```````````````````````````````````````````````````````````````
  ; Minotaur Wall Collision Detection
  ;
  dim _Bit0_P0_Col_Up = g
  dim _Bit1_P0_Col_Down = g
  dim _Bit2_P0_Col_Left = g
  dim _Bit3_P0_Col_Right = g
  dim _Bit4_P0_Dir_Up = g
...
__Main_Loop
  ...
  ;```````````````````````````````````````````````````````````````
  ;  Clears player1 direction bits since joystick has been moved.
  ;
  _BitOp_P1_P0_Dir = _BitOp_P1_P0_Dir & %11110000
...
__Skip_p0_p1_Collision
  ...
  ;```````````````````````````````````````````````````````````````
  ; IF THE CHANGE IN X REQUIRED IS GREATER, THEN GO TO THE X CHANGE
  ; otherwise do the y stuff
  ;```````````````````````````````````````````````````````````````
  if _octant{0}  then goto __Skip_Chase1
  ; change in y here
__Reenter_Chase1
  _P0_TEMPY = player0y + _Data_yinc[_octant]
  if _P0_TEMPY < 0 && _Bit0_P0_Col_Up{0} then _Bit0_P0_Col_Up{0}=0 : goto __Skip_Chase1
  if _P0_TEMPY > 0 && _Bit1_P0_Col_Down{1} then _Bit1_P0_Col_Down{1}=0 : goto __Skip_Chase1
...
;''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
; The player is farther away in the x direction
;
  ; change in x here
  _P0_TEMPX = player0x + _Data_xinc[_octant]
  if _P0_TEMPX < 0 && _Bit2_P0_Col_Left{0} then _Bit2_P0_Col_Left{0}=0 : goto __Reenter_Chase1
  if _P0_TEMPX > 0 && _Bit3_P0_Col_Right{1} then _Bit3_P0_Col_Right{1}=0 : goto __Reenter_Chase1
  _error_accumulator = _error_accumulator - _delta_y
  ; change in y here
  if temp1 < _error_accumulator then _error_accumulator = _error_accumulator + _delta_x : _P0_TEMPY = player0y + _Data_yinc[_octant]
...
__Skip_Chase2
  ...
  ; go up
  if _P0_TEMPY >= player0y then __Skip_P0_Up
  ;```````````````````````````````````````````````````````````````
  ;  Turns on the up direction bit.
  ;
  _Bit4_P0_Dir_Up{0} = 1
  ...
  ;  Skips this section if hitting the edge.
  ;
  if player0y <= _P_Edge_Top then _Bit0_P0_Col_Up{0}=1 : goto __Skip_P0_Up
  ...
  if temp5 < 34 then if pfread(temp5,temp6) then _Bit0_P0_Col_Up{0}=1 : goto __Skip_P0_Up

  temp4 = (player0x-17)/4

  if temp4 < 34 then if pfread(temp4,temp6) then _Bit0_P0_Col_Up{0}=1 : goto __Skip_P0_Up

  temp3 = temp5 - 1

  if temp3 < 34 then if pfread(temp3,temp6) then _Bit0_P0_Col_Up{0}=1 : goto __Skip_P0_Up
...
__Skip_P0_Up
  ...
  ;
  ;```````````````````````````````````````````````````````````````
  ;  Skips this section if P0 isn't moved down.
  ;
  if _P0_TEMPY <= player0y then goto __Skip_P0_Down

  ;```````````````````````````````````````````````````````````````
  ;  Turns on the down direction bit.
  ;
  _Bit5_P0_Dir_Down{1} = 1

  ;```````````````````````````````````````````````````````````````
  ;  Skips this section if hitting the edge.
  ;
  if player0y >= _P_Edge_Bottom then _Bit1_P0_Col_Down{1}=1 : goto __Skip_P0_Down
  ...
  if temp5 < 34 then if pfread(temp5,temp6) then _Bit1_P0_Col_Down{1}=1 : goto __Skip_P0_Down

  temp4 = (player0x-17)/4

  if temp4 < 34 then if pfread(temp4,temp6) then _Bit1_P0_Col_Down{1}=1 : goto __Skip_P0_Down

  temp3 = temp5 - 1

  if temp3 < 34 then if pfread(temp3,temp6) then _Bit1_P0_Col_Down{1}=1 : goto __Skip_P0_Down
...
__Skip_P0_Down
  ...
  ;
  ;```````````````````````````````````````````````````````````````
  ;  Skips this section if player0 isn't moved to the left.
  ;
  if _P0_TEMPX >= player0x then goto __Skip_P0_Left

  ;```````````````````````````````````````````````````````````````
  ;  Turns on the left direction bit.
  ;
  _Bit6_P0_Dir_Left{2} = 1

  ;```````````````````````````````````````````````````````````````
  ;  Skips this section if hitting the edge.
  ;
  if player0x <= _P_Edge_Left then _Bit2_P0_Col_Left{2}=1 : goto __Skip_P0_Left
  ...
  if temp6 < 34 then if pfread(temp6,temp5) then _Bit2_P0_Col_Left{2}=1 : goto __Skip_P0_Left

  temp3 = (player0y-8)/8

  if temp6 < 34 then if pfread(temp6,temp3) then _Bit2_P0_Col_Left{2}=1 : goto __Skip_P0_Left
...
__Skip_P0_Left
  ...
  ;  Skips this section if joystick isn't moved to the right.
  ;
  if _P0_TEMPX <= player0x then goto __Skip_P0_Right

  ;```````````````````````````````````````````````````````````````
  ;  Turns on the right direction bit.
  ;
  _Bit7_P0_Dir_Right{3} = 1

  ;```````````````````````````````````````````````````````````````
  ;  Skips this section if hitting the edge.
  ;
  if player0x >= _P_Edge_Right then _Bit3_P0_Col_Right{3}=1 : goto __Skip_P0_Right
  ...
  if temp6 < 34 then if pfread(temp6,temp5) then _Bit3_P0_Col_Right{3}=1 : goto __Skip_P0_Right

  temp3 = (player0y-8)/8

  if temp6 < 34 then if pfread(temp6,temp3) then _Bit3_P0_Col_Right{3}=1 : goto __Skip_P0_Right
